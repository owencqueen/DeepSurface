...
for train_inds, test_inds in inds[split_no]:
    ...
    training_args = TrainingArguments(
        output_dir='./models/custom-model-gen-pairs_{}'.format(RUN),
        learning_rate=2e-4,
    ...
    print('Building Model...')
    model2 = AutoModelForSequenceClassification.from_pretrained('./models/custom-model-gen-pairs_{}/checkpoint-50000'.format(RUN), num_labels=2)

    training_args = TrainingArguments(
        output_dir='./models/custom-model-fine-pairs_{}'.format(RUN),
        learning_rate=2e-5,
        per_device_train_batch_size=32,
        per_device_eval_batch_size=32,
        num_train_epochs=2,
    ...
    with open('./results/BERT-custom-DT-pairs-scores_{}.txt'.format(RUN),'w') as data: 
