...
for i in range(5):
    ...
    model = ks.Sequential()
    model.add(layers.Embedding(21, 5, mask_zero=True))
    #model.add(LSTM(10, return_sequences=True, activation='tanh', input_shape=(x_train_seq.shape[1], 1)))
    model.add(layers.LSTM(50, return_sequences=True, activation='tanh'))
    model.add(layers.LSTM(25, return_sequences=True, activation='tanh'))
